<template>
    <div class="flex flex-row items-center border-2 bg-amber-100 p-3 rounded-lg">
      <img src="@/assets/profileimg.png" style="width: 50px" alt="" />
      <div class="pr-7 pl-3">{{ userStore.loginUser.nickname }}</div>
      <div
        class="basis-1/8 p-2 border-2 border-pink-400 rounded-lg"
        @click="showFollower"
      >
        Follower : {{ followerList.length }}
      </div>
      <div
        class="basis-1/8 p-2 mx-2.5 border-2 border-indigo-400 rounded-lg"
        @click="showFollowing"
      >
